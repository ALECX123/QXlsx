...
bool Worksheet::write(int row, int column, const QVariant &value, const Format &format)
{
	...
	else if (value.userType() == QMetaType::Int || value.userType() == QMetaType::UInt || value.userType() == QMetaType::LongLong || value.userType() == QMetaType::ULongLong || value.userType() == QMetaType::Double || value.userType() == QMetaType::Float)
	{
		// Number
...
bool Worksheet::copyRichString(Document& src,int src_row, int src_column, int dst_row, int dst_column, const RichString &value, const Format &format)
{
	Q_D(Worksheet);
	//    QString content = value.toPlainString();
	if (d->checkDimensions(src_row, src_column))
		return false;
	...
	Format fmt = format.isValid() ? format : src.cellAt(src_row, src_column)->format();
	qInfo()<<"d->sharedStrings()->count():"<<d->sharedStrings()->count()<<fmt.fontName()<< value.toString();
	// if (value.toString().isEmpty())
	// {
	// 	//not write richstring
	// 	return false;
	// }
	...
	QSharedPointer<Cell> cell = QSharedPointer<Cell>(new Cell(value.toPlainString(), Cell::SharedStringType, fmt, this));
	cell->d_ptr->richString = value;
	d->cellTable[dst_row][dst_column] = cell;
	return true;
}

QList<CellRange> Worksheet::Getmerges()
{
	Q_D(Worksheet);
	return d->merges;
}
	...
	while (!reader.atEnd() && !(reader.name() == QLatin1String("sheetData") && reader.tokenType() == QXmlStreamReader::EndElement))
	{
		if (reader.readNextStartElement())
		{
			if (reader.name() == QLatin1String("row"))
			{
				...
				if (attributes.hasAttribute(QLatin1String("customFormat")) || attributes.hasAttribute(QLatin1String("customHeight")) || attributes.hasAttribute(QLatin1String("hidden")) || attributes.hasAttribute(QLatin1String("outlineLevel")) || attributes.hasAttribute(QLatin1String("collapsed")))
				{
...
void WorksheetPrivate::loadXmlDataValidations(QXmlStreamReader &reader)
{
	...
	{
		reader.readNextStartElement();
		if (reader.tokenType() == QXmlStreamReader::StartElement && reader.name() == QLatin1String("dataValidation"))
		{
