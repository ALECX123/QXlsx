...
void Chart::addSeries(const CellReference& seriestx_title, const CellRange& seriescat_row, const CellRange& seriesval_column, AbstractSheet* sheet)
{
	Q_D(Chart);
	if (!seriestx_title.isValid() || !seriescat_row.isValid() || !seriesval_column.isValid())
		return;
	...
	{
		QSharedPointer<XlsxSeries_tcv> series = QSharedPointer<XlsxSeries_tcv>(new XlsxSeries_tcv);
		series->numRef_tx = sheetName + QLatin1String("!") + seriestx_title.toString(true, true);
		series->numRef_cat = sheetName + QLatin1String("!") + seriescat_row.toString(true, true);
		series->numRef_val = sheetName + QLatin1String("!") + seriesval_column.toString(true, true);
		d->seriesList_tcv.append(series);
	}
}
...
void ChartPrivate::saveXmlPieChart(QXmlStreamWriter& writer) const
{
	...
	for (int i = 0; i < seriesList_tcv.size(); ++i)
		saveXmlSer(writer, seriesList_tcv[i].data(), i);
...
void ChartPrivate::saveXmlSer(QXmlStreamWriter& writer, XlsxSeries_tcv* ser, int id) const
{
	...
	if (!ser->numRef_tx.isEmpty())
	{
		writer.writeStartElement(QStringLiteral("c:tx"));
		writer.writeStartElement(QStringLiteral("c:strRef"));
		writer.writeTextElement(QStringLiteral("c:f"), ser->numRef_tx);
		writer.writeEndElement();
		writer.writeEndElement();
	}
	if (!ser->numRef_cat.isEmpty())
	{
		writer.writeStartElement(QStringLiteral("c:cat"));
		writer.writeStartElement(QStringLiteral("c:strRef"));
		writer.writeTextElement(QStringLiteral("c:f"), ser->numRef_cat);
		writer.writeEndElement();
		writer.writeEndElement();
	}
	if (!ser->numRef_val.isEmpty())
	{
		...
		writer.writeTextElement(QStringLiteral("c:f"), ser->numRef_val);
