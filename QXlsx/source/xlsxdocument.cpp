	...
	std::string copyTag(const std::string &sFrom, const std::string &sTo, const std::string &tag)
	{
		...
		while (true)
		{
			...
			if (startPos != std::string::npos)
			{
				...
				{ // second try to find the ending, maybe it is "/>"
...
bool DocumentPrivate::copyStyle(const QString &from, const QString &to)
{
	...
	for (int i = 0; i < toFilePaths.size(); i++)
	{
		...
		{
			if (filePaths.contains(toFilePaths[i]))
			{ // style file exist in 'from' as well
				// modify style file
		...
		{
			if (filePaths.contains(toFilePaths[i]))
			{ // workbook file exist in 'from' as well
		...
		{
			if (filePaths.contains(toFilePaths[i]))
			{ // sheet file exist in 'from' as well
				// modify sheet file
				std::string fromData = QString::fromUtf8(zipReader.fileData(toFilePaths[i])).toStdString();
				std::string toData = QString::fromUtf8(toReader->fileData(toFilePaths[i])).toStdString();
...
bool Document::CopyCell(const QXlsx::CellRange& srcrange, const QXlsx::CellReference& dstrange, QXlsx::Document& src, QXlsx::Document& dst)
{
	// mergeCell Copy
	for (int listi = 0; listi < src.currentWorksheet()->mergedCells().size(); listi++)
	{
		QXlsx::CellRange _CellRangeMerged = src.currentWorksheet()->mergedCells().at(listi);
		if (_CellRangeMerged.firstRow() >= srcrange.firstRow() && _CellRangeMerged.firstColumn() >= srcrange.firstColumn() &&
			_CellRangeMerged.lastRow() <= srcrange.lastRow() && _CellRangeMerged.lastColumn() <= srcrange.lastColumn())
		{
			_CellRangeMerged.setFirstRow(_CellRangeMerged.firstRow() + dstrange.row() - srcrange.firstRow());
			_CellRangeMerged.setLastRow(_CellRangeMerged.lastRow() + dstrange.row() - srcrange.firstRow());
			_CellRangeMerged.setFirstColumn(_CellRangeMerged.firstColumn() + dstrange.column() - srcrange.firstColumn());
			_CellRangeMerged.setLastColumn(_CellRangeMerged.lastColumn() + dstrange.column() - srcrange.firstColumn());
			dst.mergeCells(_CellRangeMerged /*,src.cellAt(row, col)->format()*/);
		}
	}

	//copy style
	struct CopyCellStyle
	{
		int RowHeight;
		QXlsx::Format cell_format;
		QVariant Cellvalue;
	};
	for (int row = srcrange.firstRow(); row < srcrange.lastRow() + 1; row++)
	{
		dst.setRowHeight(dstrange.row() + row - srcrange.firstRow(), src.rowHeight(row));
		CopyCellStyle _CopyCellStyle;
		_CopyCellStyle.RowHeight = row;
		for (int col = srcrange.firstColumn(); col < srcrange.lastColumn() + 1; col++)
		{
			_CopyCellStyle.Cellvalue = src.cellAt(row, col)->value();
			_CopyCellStyle.cell_format.setFontSize(src.cellAt(row, col)->format().fontSize());
			_CopyCellStyle.cell_format.setTextWrap(src.cellAt(row, col)->format().textWrap());
			_CopyCellStyle.cell_format.setFont(src.cellAt(row, col)->format().font());
			_CopyCellStyle.cell_format.setLeftBorderStyle(src.cellAt(row, col)->format().leftBorderStyle());
			_CopyCellStyle.cell_format.setRightBorderStyle(src.cellAt(row, col)->format().rightBorderStyle());
			_CopyCellStyle.cell_format.setBottomBorderStyle(src.cellAt(row, col)->format().bottomBorderStyle());
			_CopyCellStyle.cell_format.setTopBorderStyle(src.cellAt(row, col)->format().topBorderStyle());
			_CopyCellStyle.cell_format.setHorizontalAlignment(src.cellAt(row, col)->format().horizontalAlignment());
			_CopyCellStyle.cell_format.setVerticalAlignment(src.cellAt(row, col)->format().verticalAlignment());
			dst.write(dstrange.row() + row - srcrange.firstRow(),
				dstrange.column() + col - srcrange.firstColumn(),
				_CopyCellStyle.Cellvalue,
				_CopyCellStyle.cell_format);
		}
	}
	return true;
}
